import { AIPromptEngine, AIAnalysisRequest, AIAnalysisResponse, TerraformContext } from './aiPromptEngine';
...
export class CostAnalysisService {
    private apiEndpoint = process.env.CODEGUARD_API_ENDPOINT || '';

    public async analyze(terraformCode: string): Promise<CostAnalysis> {
    ...
    private parseTerraformBlock(code: string): TerraformContext {
    ...
    private async callBedrockAPI(context: TerraformContext): Promise<CostAnalysis> {
        const request: AIAnalysisRequest = {
            context,
            analysisType: 'cost_optimization'
        };

        const prompt = AIPromptEngine.generatePrompt(request);
    ...
    private convertAIResponseToUI(aiResponse: AIAnalysisResponse): CostAnalysis {
    ...
    private getEnhancedMockAnalysis(context: TerraformContext): CostAnalysis {
    ...
    private getNATGatewayAnalysis(): CostAnalysis {
        return {
...
resource "aws_vpc_endpoint" "dynamodb" {
  ...
  tags = {
    ...
    private getDynamoDBAnalysis(): CostAnalysis {
        return {
  ...
  tags = {
    ...
    private getLambdaMemoryAnalysis(): CostAnalysis {
        return {
  ...
  environment {
    ...
    private getEC2InstanceAnalysis(): CostAnalysis {
        return {
  ...
  tags = {
    ...
    private getRDSInstanceAnalysis(): CostAnalysis {
        return {
  ...
  # ~$43/month for low usage
    ...
    private getEBSVolumeTypeAnalysis(): CostAnalysis {
        return {
  ...
  throughput = 125
    ...
    private getEBSVolumeSizeAnalysis(): CostAnalysis {
        return {
...
resource "aws_s3_bucket" "archive" {
  ...
  lifecycle_rule {
    ...
    private getALBAnalysis(): CostAnalysis {
        return {
...
# vs ALB: $22/month + $0.008 per LCU-hour`,
    ...
    private getEIPAnalysis(): CostAnalysis {
        return {
...
# (No EIP needed - ALB provides DNS name)`,
    ...
    private getCloudWatchRetentionAnalysis(): CostAnalysis {
        return {
...
resource "aws_cloudwatch_log_subscription_filter" "export" {
  ...
  destination_arn = aws_kinesis_firehose_delivery_stream.logs.arn
    ...
    private getS3StorageClassAnalysis(): CostAnalysis {
        return {
  ...
  rule {
    ...
    private getRDSStorageAnalysis(): CostAnalysis {
        return {
  ...
  # Storage autoscaling enabled
    ...
    private getMultiAZAnalysis(): CostAnalysis {
        return {
  ...
  tags = {
    ...
    private getEC2MonitoringAnalysis(): CostAnalysis {
        return {
  ...
  # - Free tier eligible
    ...
    private getGenericMockAnalysis(): CostAnalysis {
        return {
            currentCost: 'Unknown',
